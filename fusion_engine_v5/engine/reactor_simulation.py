...
def _monte_carlo_plasma(tct: Dict[str, Any], samples: int = 30000) -> Dict[str, Any]:
    ...
    stability = _safe_float(
        tct.get("stability", {}).get("stability", tct.get("stability_metric", 0.85)),
        0.85,
    )
    ...
    # use a deterministic expectation rather than a random binomial so runs stay reproducible
...
def simulate_reactor(design: Dict[str, Any], blanket_validate: bool = False) -> Dict[str, Any]:
    plasma = evaluate_case(
        _safe_float(design["R"]),
        _safe_float(design["a"]),
    ...
    if blanket_validate:
        blanket = run_openmc_validation(design, plasma)
        if blanket is None:
            blanket = _call_evaluate_blanket(design, plasma)
            blanket["model"] = "dataset_fallback"
    else:
        blanket = _call_evaluate_blanket(design, plasma)

    blanket = dict(blanket)
    blanket.setdefault("model", "dataset")
    ...
    size_penalty = (
        50.0 * max(_safe_float(design["R"]) - 8.5, 0.0) ** 2
        + 45.0 * max(_safe_float(design["a"]) - 2.6, 0.0) ** 2
    )

    grid_penalty = 0.0
    net_electric = _safe_float(plant.get("net_electric", 0.0), 0.0)
    ...
    complexity_penalty = (
        15.0 * max(_safe_float(design["B0"]) - 8.0, 0.0) ** 2
        + 3.0 * max(_safe_float(design["lithium_velocity"]) - 4.0, 0.0) ** 2
        + 80.0 * max(_safe_float(design["blanket_thickness"]) - 1.2, 0.0) ** 2
    )

    breeding_penalty = 0.0
    if _safe_float(blanket["TBR"]) < 1.15:
        breeding_penalty += 15000.0 * (1.15 - _safe_float(blanket["TBR"]))

    wall_penalty = 0.0
    if effective_wall_load > 6.5:
        wall_penalty += 30000.0 * (effective_wall_load - 6.5) ** 2
    ...
    ip_ma = max(_safe_float(plasma.get("Ip_MA", _safe_float(design["Ip"], 10.0))), 0.1)
    betan = beta * _safe_float(design["B0"]) * _safe_float(design["a"]) / ip_ma
    ...
    if _safe_float(blanket.get("attenuation", 0.0)) < 0.05:
        hard_fail_penalty += 300000.0 + 100000.0 * (0.05 - _safe_float(blanket.get("attenuation", 0.0)))

    # keep "reactor-like" regimes preferred without outright forbidding smaller cases
    if net_electric < 1500.0:
        hard_fail_penalty += 50000.0 + 500.0 * (1500.0 - net_electric)

    if _safe_float(design["R"]) > 12.0:
        hard_fail_penalty += 100000.0 + 25000.0 * (_safe_float(design["R"]) - 12.0)
    if _safe_float(design["a"]) > 2.5:
        hard_fail_penalty += 100000.0 + 25000.0 * (_safe_float(design["a"]) - 2.5)
    if _safe_float(design["B0"]) > 9.5:
        hard_fail_penalty += 100000.0 + 25000.0 * (_safe_float(design["B0"]) - 9.5)

    performance_boost = lithium_mod
    bootstrap_boost = bootstrap_frac

    score = (
        2000.0 * float(np.asarray(bootstrap_frac).reshape(-1)[0])
        + annual_revenue_musd
        - annual_cost_musd
        - penalty_stress
        - grid_penalty
        ...
        - hard_fail_penalty
        - stability_penalty
        + 600.0 * _safe_float(tct.get("stability", {}).get("stability", 0.0), 0.0)
        - 2000.0 * _safe_float(mc["fail_rate"], 0.0)
    )

    return {
        "score": float(score),
        "net_electric": float(net_electric),
        "TBR": float(_safe_float(blanket["TBR"])),
        "fail_rate": float(_safe_float(mc["fail_rate"])),
        ...
        "wall_temp": float(_safe_float(wall_temp)),
        "blanket_model": blanket.get("model", "unknown"),
        "blanket_attenuation": float(_safe_float(blanket.get("attenuation", 0.0))),
        "blanket_front_heating_frac": float(_safe_float(blanket.get("front_heating_frac", 1.0))),
        "capex_billion": float(capex_billion),
        "annual_revenue_musd": float(annual_revenue_musd),
        "annual_cost_musd": float(annual_cost_musd),
        "mc_samples": int(mc["samples"]),
        "tct_control_strength": float(_safe_float(tct.get("control_strength", 0.0))),
        "tct_precursor": float(_safe_float(tct.get("precursor", 0.0))),
        "tct_upstream_factor": float(_safe_float(tct.get("upstream_factor", 1.0))),
        "tct_risk_R": float(_safe_float(tct.get("risk_R", 0.0))),
        "tct_risk_H": float(_safe_float(tct.get("risk_H", 0.0))),
        "tct_n_used": float(_safe_float(tct.get("n_used", plasma.get("n_e3", plasma.get("n_e", 0.0))))),
        "tct_B_used": float(_safe_float(tct.get("B_used", plasma.get("B_t", 0.0)))),
        "tct_Te_used": float(_safe_float(tct.get("Te_used", plasma.get("T_keV", plasma.get("Te_keV", 0.0))))),
    }
