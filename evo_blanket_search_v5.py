...
BASE_DIR = Path.cwd()
RUNS_DIR = BASE_DIR / "runs_evo_blanket_v5"
...
# Li-6 enrichment choices for breeder-type layers
LI6_ENRICH_CHOICES = [0.30, 0.60, 0.90]

# Packing / porosity factor for porous breeder / multiplier ceramics
...
def build_material(name: str, li6_enrich: float = 0.90, packing: float = 1.0) -> openmc.Material:
    ...
    # Rebuild lithium pair using requested enrichment
    li6_base = 0.0
    li7_base = 0.0
    for kind, sym, frac, pct in spec["components"]:
...
def postprocess_case(run_dir: Path, design: dict) -> dict:
    ...
    h3_l1 = float(h3_prod[0]) if len(h3_prod) > 0 else 0.0
    h3_l2 = float(h3_prod[1]) if len(h3_prod) > 1 else 0.0
    h3_l3 = float(h3_prod[2]) if len(h3_prod) > 2 else 0.0
    h3_l4 = float(h3_prod[3]) if len(h3_prod) > 3 else 0.0
    h3_total = h3_l1 + h3_l2 + h3_l3 + h3_l4
    h3_weighted = 1.00 * h3_l1 + 0.95 * h3_l2 + 0.80 * h3_l3 + 0.60 * h3_l4

    heat_l1 = float(heating[0]) if len(heating) > 0 else 0.0
    heat_l2 = float(heating[1]) if len(heating) > 1 else 0.0
    heat_l3 = float(heating[2]) if len(heating) > 2 else 0.0
    heat_l4 = float(heating[3]) if len(heating) > 3 else 0.0
    heat_total = heat_l1 + heat_l2 + heat_l3 + heat_l4

    heat_frac_l1 = heat_l1 / heat_total if heat_total > 0 else 0.0
    heat_frac_l2 = heat_l2 / heat_total if heat_total > 0 else 0.0
    heat_frac_l3 = heat_l3 / heat_total if heat_total > 0 else 0.0
    heat_frac_l4 = heat_l4 / heat_total if heat_total > 0 else 0.0
    ...
    # Strong reward for tritium production
    score /= (1.0 + 200.0 * h3_weighted)

    # Stronger penalty for front-wall heating
    score *= (1.0 + 1.30 * heat_frac_l1)

    # Mild reward for deeper heat deposition
    score /= (1.0 + 0.20 * (heat_frac_l3 + heat_frac_l4))
    ...
    return {
        ...
        "heat_frac_l2": heat_frac_l2,
        "heat_frac_l3": heat_frac_l3,
        "heat_frac_l4": heat_frac_l4,
        "score": score,
    }
...
def default_li6(material_name: str) -> float:
    return 0.90 if material_uses_lithium(material_name) else 0.0


def default_pack(material_name: str) -> float:
    return 0.80 if MATERIALS[material_name].get("porous_ok", False) else 1.0


def random_layer() -> tuple[str, float, float]:
    mat = random.choice(CANDIDATES)
    li6 = random.choice(LI6_ENRICH_CHOICES) if material_uses_lithium(mat) else 0.0
    pack = random.choice(PACKING_CHOICES) if MATERIALS[mat].get("porous_ok", False) else 1.0
    return mat, li6, pack
...
def mutate_design(parent: dict) -> dict:
    ...
    elif choice.startswith("li6_"):
        idx = choice[-1]
        lname = f"l{idx}"
        if material_uses_lithium(child[lname]):
            child[f"{lname}_li6"] = random.choice(LI6_ENRICH_CHOICES)
    elif choice.startswith("pack_"):
        idx = choice[-1]
        lname = f"l{idx}"
        if MATERIALS[child[lname]].get("porous_ok", False):
            child[f"{lname}_pack"] = random.choice(PACKING_CHOICES)
    elif choice == "total_cm":
        child["total_cm"] = random.choice(TOTAL_THICKNESS_CHOICES)
    elif choice == "split":
        child["split"] = random.choice(SPLIT_CHOICES)
    elif choice == "swap":
        a, b = random.sample(["l1", "l2", "l3", "l4"], 2)
        for suffix in ("", "_li6", "_pack"):
            child[a + suffix], child[b + suffix] = child[b + suffix], child[a + suffix]
...
def design_key(d: dict):
    return (
        d["l1"], d["l1_li6"], d["l1_pack"],
        d["l2"], d["l2_li6"], d["l2_pack"],
        d["l3"], d["l3_li6"], d["l3_pack"],
        d["l4"], d["l4_li6"], d["l4_pack"],
        d["total_cm"], d["split"]
    )
...
def run_design(design: dict, generation: int, idx: int) -> dict:
    ...
    row = {
        ...
        "run_name": run_name,
        "l1": design["l1"], "l1_li6": design["l1_li6"], "l1_pack": design["l1_pack"],
        "l2": design["l2"], "l2_li6": design["l2_li6"], "l2_pack": design["l2_pack"],
        "l3": design["l3"], "l3_li6": design["l3_li6"], "l3_pack": design["l3_pack"],
        "l4": design["l4"], "l4_li6": design["l4_li6"], "l4_pack": design["l4_pack"],
    ...
    if ok:
        try:
            ...
            print(
                ...
                f"h3_total={row['h3_total']:.6g}, "
                f"heat_l1={row['heat_frac_l1']:.6g}"
            )
...
def row_to_design(row: dict) -> dict:
    return {
        "l1": row["l1"], "l1_li6": float(row["l1_li6"]), "l1_pack": float(row["l1_pack"]),
        "l2": row["l2"], "l2_li6": float(row["l2_li6"]), "l2_pack": float(row["l2_pack"]),
        "l3": row["l3"], "l3_li6": float(row["l3_li6"]), "l3_pack": float(row["l3_pack"]),
        "l4": row["l4"], "l4_li6": float(row["l4_li6"]), "l4_pack": float(row["l4_pack"]),
        "total_cm": float(row["total_thickness_cm"]),
        "split": tuple(float(x) / 100.0 for x in row["split"].split("_")),
    }
...
def main() -> None:
    ...
    for gen in range(GENERATIONS):
        ...
        for i, design in enumerate(population):
            ...
            gen_rows.append(row)
            all_rows.append(row)
            write_csv(BASE_DIR / "evo_blanket_results_v5.csv", all_rows)
    ...
    for r in final_good[:25]:
        print(
            ...
            f"h3_total={r['h3_total']:.6g} | "
            f"heat_l1={r['heat_frac_l1']:.6g}"
        )

    print("\nFinished. Wrote evo_blanket_results_v5.csv")
