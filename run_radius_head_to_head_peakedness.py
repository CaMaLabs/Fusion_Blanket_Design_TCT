...
def deposition_peakedness(openmc_res):
    """
    First-principles-ish proxy from available OpenMC-side outputs.
    Uses:
    - front heating fraction
    - attenuation
    - gradient
    and interprets strong simultaneous saturation as possible front/mid-zone concentration.
    """
    front_heat = f(openmc_res.get("front_heating_frac", 0.0))
    attn = f(openmc_res.get("attenuation", openmc_res.get("ATTN", 0.0)))
    grad = f(openmc_res.get("gradient", openmc_res.get("GRAD", 0.0)))

    # baseline front-heating expectation
    front_excess = max(0.0, front_heat - 0.16)

    # if both attenuation and gradient are near 1, that can mean
    # energy is being trapped very effectively; penalize only mildly.
    trap_factor = clamp((attn - 0.995) / 0.005, 0.0, 1.0) * clamp((grad - 0.995) / 0.005, 0.0, 1.0)

    # peakedness score: excess front-heating amplified by saturation regime
    peakedness = clamp(front_excess * (1.0 + 1.5 * trap_factor), 0.0, 0.25)

    return {
        "front_excess": front_excess,
        "trap_factor": trap_factor,
        "peakedness_penalty": peakedness,
        "front_heat_adjusted": front_heat + peakedness,
    }


def engineering_score(row):
    """
    Compare candidates with a neutronics guardrail but penalize thermal concentration.
    Higher is better.
    """
    score = 0.0
    score += 60.0 * row["TBR_openmc"]
    score += 8.0 * row["ATTN_openmc"]
    score += 8.0 * row["GRAD_openmc"]
    score += 0.02 * row["net_electric"]

    score -= 120.0 * row["front_heat_adjusted"]
    score -= 0.08 * row["wall_load_corrected"]
    score -= 0.0015 * row["wall_temp_corrected"]
    score -= 600.0 * row["fail_rate_corrected"]
    score -= 150.0 * row["peakedness_penalty"]
...
for radius_cm in RADII:
    ...
    peak = deposition_peakedness(openmc_res)
    ...
    row = {
        ...
        "front_heat_adjusted": f(peak.get("front_heat_adjusted", 0.0)),
        "front_excess": f(peak.get("front_excess", 0.0)),
        "trap_factor": f(peak.get("trap_factor", 0.0)),
        "peakedness_penalty": f(peak.get("peakedness_penalty", 0.0)),
        "wall_load_raw": f(reactor_res.get("raw_wall_load", reactor_res.get("wall_load", 0.0))),
    ...
    row["engineering_score"] = engineering_score(row)
    rows.append(row)

    for k, v in row.items():
        print(f"{k}: {v}")

ranked = sorted(rows, key=lambda r: r["engineering_score"], reverse=True)

print("\n=== RANKED SUMMARY ===")
for r in ranked:
    print(
        "radius_cm=", r["radius_cm"],
        "eng_score=", r["engineering_score"],
        "TBR=", r["TBR_openmc"],
        "ATTN=", r["ATTN_openmc"],
        "GRAD=", r["GRAD_openmc"],
        "front_raw=", r["front_heating_raw"],
        "front_adj=", r["front_heat_adjusted"],
        "peak_pen=", r["peakedness_penalty"],
...
out = ROOT / "radius_head_to_head_peakedness_result.json"
out.write_text(json.dumps({"rows": rows, "ranked": ranked}, indent=2), encoding="utf-8")
print("\nSaved radius_head_to_head_peakedness_result.json")
