...
| TCT control response | Reduced-model BOUT++ plus HW2D-style cross-code support; closed-loop threshold result is mixed | `bout_tct_current_sheet_sweep.py`, `bout_tct_actuator_robustness_sweep.py`, `bout_hw_turbulence_sweep.py`, `hw2d_cross_validation.py`, `closed_loop_tct_validation.py`, `validation_runs/closed_loop_tct_validation_default/` | Replace the late max-vorticity trigger with a predictive growth-rate or sheet-thinning precursor, then map it onto machine-equilibrium cases. |
