...
| TCT control response | Screening-level / needs benchmark; shot-specific DIII-D diagnostic reconstruction and authorized-data replacement contract prepared | TCT proxy scripts; `diiid_diagnostic_reconstruction.py`; `diiid_fdp_fetch_template.py`; `validation_runs/diiid_diagnostic_reconstruction_default/` | Obtain limited read-only DIII-D/FDP access and replace reconstructed precursor traces with magnetic, ECE, density, event-marker, and actuator channels. |
