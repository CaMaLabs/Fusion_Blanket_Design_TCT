...
| TCT control response | Reduced-model BOUT++ support | `bout_tct_current_sheet_sweep.py`, `bout_tct_actuator_robustness_sweep.py`, `validation_runs/bout_tct_actuator_robustness_default/` | Add closed-loop preemptive triggering and map it onto machine-equilibrium cases. |
...
| Machine EFIT / GEQDSK inputs | Partial | DIII-D public EFIT `g/a/p` package copied from CaMaLabs/M3DC1; NSTX-U and ITER templates found but no public EFIT GEQDSK in the scanned package | Add real NSTX-U and ITER equilibrium files; require experimental diagnostics before promoting beyond reduced-model status. |
