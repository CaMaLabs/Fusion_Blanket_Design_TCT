...
| TCT control response | Reduced-model BOUT++ plus HW2D-style cross-code support; predictive BOUT++ closed-loop trigger supported with HW2D limitation | `bout_tct_current_sheet_sweep.py`, `bout_tct_actuator_robustness_sweep.py`, `bout_hw_turbulence_sweep.py`, `hw2d_cross_validation.py`, `closed_loop_tct_validation.py`, `predictive_tct_validation.py`, `validation_runs/predictive_tct_validation_default/` | Build a HW2D delayed-growth precursor case or sheet-thinning trigger, then map the predictive controller onto machine-equilibrium cases. |
