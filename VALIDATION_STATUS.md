...
|---|---|---|---|
| Plasma operating point | Proxy / analytic model | `fusion_engine_v5/engine/plasma_model.py`, `fusion_engine_v5/engine/reactor_simulation.py` | Compare against a higher-fidelity equilibrium / transport workflow. |
| TCT control response | Hypothesis-level proxy | `fusion_engine_v5/engine/tct_control.py` | Calibrate against M3D-C1, JOREK, or published edge/reconnection simulations. |
| Current-sheet / plasmoid suppression | Hypothesis-level proxy | `current_sheet_thickness()`, `plasmoid_growth_rate()`, `ctli_stability_metric()` | Replace heuristic growth-rate reduction with validated MHD response data. |
