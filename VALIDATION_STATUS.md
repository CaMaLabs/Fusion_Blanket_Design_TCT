...
|---|---|---|---|
| Plasma operating point | Proxy / analytic model plus DIII-D EFIT-backed reduced R-Z validation; DIII-D field-aligned mesh ready | `fusion_engine_v5/engine/plasma_model.py`, `validation_inputs/machine_equilibria/`, `efit_predictive_tct_validation.py`, `diiid_hypnotoad_mesh.py`, `validation_runs/diiid_hypnotoad_mesh_default/` | Run a BOUT++ machine-geometry physics case on the DIII-D mesh; add NSTX-U and ITER EFIT/GEQDSK files. |
| TCT control response | Predictive trigger supported across reduced BOUT++ current-sheet, HW2D-style delayed-growth, and DIII-D EFIT-grid R-Z cases | `predictive_tct_validation.py`, `hw2d_delayed_growth_validation.py`, `efit_predictive_tct_validation.py`, `validation_runs/efit_predictive_tct_validation_default/` | Replace synthetic precursor drive with diagnostic-anchored timing and map the controller onto the field-aligned DIII-D mesh. |
...
| Machine EFIT / GEQDSK inputs | DIII-D baseline imported, used by a reduced R-Z predictive-control case, and converted to a validated Hypnotoad field-aligned mesh | DIII-D public EFIT `g/a/p` package copied from CaMaLabs/M3DC1; `validation_inputs/geqdsk_efit_baseline/diii_d_158103_03796/`; `validation_runs/diiid_hypnotoad_mesh_default/`; NSTX-U and ITER templates found but no public EFIT GEQDSK in the scanned package | Run BOUT++ on the DIII-D mesh, add real NSTX-U and ITER equilibrium files, and require experimental diagnostics before promoting beyond reduced-model status. |
