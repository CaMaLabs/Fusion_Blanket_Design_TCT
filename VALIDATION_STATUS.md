...
|---|---|---|---|
| Plasma operating point | Proxy / analytic model plus DIII-D EFIT-backed reduced R-Z validation; official BOUT++ `elm-pb` machine-geometry startup smoke passed with serial topology override | `fusion_engine_v5/engine/plasma_model.py`, `efit_predictive_tct_validation.py`, `diiid_hypnotoad_mesh.py`, `diiid_bout_elm_mesh_smoke.py`, `validation_runs/diiid_bout_elm_mesh_smoke_default/` | Run the exact X-point topology with 14 MPI ranks, add `Jpar0`, and perform linear growth-rate checks; add NSTX-U and ITER EFIT/GEQDSK files. |
...
| Machine EFIT / GEQDSK inputs | DIII-D baseline imported, converted to a validated Hypnotoad mesh, and ingested by official BOUT++ `elm-pb`; exact X-point evolution remains blocked by local MPI launch restrictions | DIII-D public EFIT `g/a/p` package copied from CaMaLabs/M3DC1; `validation_inputs/geqdsk_efit_baseline/diii_d_158103_03796/`; `validation_runs/diiid_hypnotoad_mesh_default/`; `validation_runs/diiid_bout_elm_mesh_smoke_default/` | Run exact topology on a conventional Linux/HPC host, add real NSTX-U and ITER equilibrium files, and require experimental diagnostics before promoting beyond reduced-model status. |
