...
python3 -u tools/tct_mechanism_explorer/native_preemptive_two_profile_handoff_audit.py
RC=$?

OUT="validation_runs/m3dc1_tct_native_preemptive_two_profile_handoff"
SUMMARY="$OUT/native_two_profile_handoff_summary.json"

echo
if [[ -f "$SUMMARY" ]]; then
  echo "===== NATIVE PREEMPTIVE TWO-PROFILE HANDOFF RESULT ====="
  jq '{
    classification,
    ...
    best_case: (.best_case | {
      case, second_start, second_shoulder_width, second_shoulder_delta,
      ...
      max_high_J_change_pct, max_high_J_time,
      t0p13_width_gain_pct, t0p13_Jpk_change_pct,
  ...
  jq '[.case_summaries[] | select(
      .t0p14_Jpk_change_pct <= 0.10 and .t0p14_width_gain_pct > 0 and
      .t0p15_Jpk_change_pct <= 0.10 and .t0p15_width_gain_pct > 0 and
      .t0p16_Jpk_change_pct <= 0.10 and .t0p16_width_gain_pct > 0)
    | {case, second_start,
       t0p14_width_gain_pct, t0p14_Jpk_change_pct,
       t0p15_width_gain_pct, t0p15_Jpk_change_pct,
       t0p16_width_gain_pct, t0p16_Jpk_change_pct,
       final_width_gain_pct}]' "$SUMMARY"
