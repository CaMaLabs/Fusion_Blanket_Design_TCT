...
def install_operator() -> bool:
    ...
    if "mag_ctrl_period" not in text:
        needle = "  real :: mag_ctrl_t_off  ! magnetic control turn-off time\n"
        if needle not in text:
            raise RuntimeError("imag_control variables not found in M3Dmodules.f90")
        text = text.replace(
            needle,
            needle
            + "  real :: mag_ctrl_period  ! repeated pulse period; <=0 means single gate\n"
            + "  real :: mag_ctrl_pulse_width ! on-time within each repeated pulse period\n",
            1,
        )
    ...
    if "mag_phase" not in text:
        old_decl = "  real :: mag_gate, mag_tau, mag_wr, mag_wz\n"
        if old_decl not in text:
            raise RuntimeError("imag_control local declaration not found in ludef_t.f90")
        text = text.replace(old_decl, "  real :: mag_gate, mag_tau, mag_wr, mag_wz, mag_phase\n", 1)
        changed = True
