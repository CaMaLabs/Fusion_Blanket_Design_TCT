...
echo
if [[ -f "$SUMMARY" ]]; then
  echo "===== NATIVE PREEMPTIVE-HANDOFF RESULT ====="
  jq '{
    classification,
    ...
    best_case: (
      ...
      | {
          ...
          max_high_J_time,
          t0p12_width_gain_pct,
          t0p12_Jpk_change_pct,
  ...
  echo
  echo "===== CASES SAFE THROUGH t=0.14 ====="
  jq '[.case_summaries[]
    | select(
        .t0p14_width_gain_pct > 0
        and .t0p14_Jpk_change_pct <= 0.10
      )
    | {
        case,
        handoff_time,
  ...
  echo "Inspect:"
  echo "/tmp/m3dc1_tct_native_preemptive_handoff_refinement_runs"
fi
