...
python3 -u tools/tct_mechanism_explorer/native_earliest_timing_weak_amplitude_refinement_audit.py
RC=$?

OUT="validation_runs/m3dc1_tct_native_earliest_timing_weak_amplitude_refinement"
SUMMARY="$OUT/preemptive_timing_amplitude_refinement_summary.json"

echo
if [[ -f "$SUMMARY" ]]; then
  echo "===== NATIVE EARLIEST TIMING / WEAK-AMPLITUDE REFINEMENT ====="
  jq '{
    classification,
  ...
  echo
  echo "===== EARLIEST-TIMING RANKING ====="
  jq '[.case_summaries[] | {
    case, second_start, second_amp,
    sustained_safe_authority,
    continuous_positive_width_t0p10_to_t0p30,
    current_gate_pass_every_step_t0p10_to_t0p30,
    width_gate_pass_any,
