...
def compare_series(baseline: list[dict[str, float]], controlled: list[dict[str, float]],
                   active_start: float, active_end: float, response_horizon: float = 0.05,
                   time_tolerance: float = 1e-9) -> dict[str, float]:
    ...
    for c in controlled:
        ...
        if delta <= time_tolerance:
            pairs.append((baseline[i], c)); unused.remove(i)
    if not pairs:
        raise RuntimeError("no equal-physical-time baseline/control samples")
    response_end = active_end + max(response_horizon, 0.0)
    response = [(b, c) for b, c in pairs if c["time"] > active_start + time_tolerance
                and c["time"] <= response_end + time_tolerance]
    if not response:
        response = [(b, c) for b, c in pairs if c["time"] > active_start + time_tolerance][:1] or pairs[-1:]

    samples = []
    for b, c in response:
        samples.append({"time": c["time"], "width": _pct(c["W_sheet"], b["W_sheet"]),
            "jpk": _pct(c["Jpk"], b["Jpk"]), "high_j": _pct(c["Jint_high"], b["Jint_high"]),
    ...
    integral = sum(0.5 * (a["width"] + b["width"]) * (b["time"] - a["time"])
                   for a, b in zip(samples, samples[1:]))
    current_changes = [abs(_pct(c["toroidal_current"], b["toroidal_current"])) for b, c in response
                       if math.isfinite(c["toroidal_current"]) and math.isfinite(b["toroidal_current"])
                       and abs(b["toroidal_current"]) > 1e-300]
    def recon_rate(series):
        rates = [abs((b["Reconnected_Flux"] - a["Reconnected_Flux"]) / (b["time"] - a["time"]))
                 for a, b in zip(series, series[1:]) if b["time"] != a["time"]
                 and math.isfinite(a["Reconnected_Flux"]) and math.isfinite(b["Reconnected_Flux"])]
        return max(rates, default=0.0)
    final_b, final_c = pairs[-1]
    return {"impulse_sample_count": len(samples), "impulse_window_start": active_start,
        "impulse_window_end": response_end, "immediate_width_gain_pct": widths[0],
        "peak_favorable_width_gain_pct": peak["width"], "peak_favorable_response_time": peak["time"],
        ...
        "response_latency": peak["time"] - active_start, "response_duration": duration,
        ...
        "mean_active_width_gain_pct": sum(widths) / len(widths), "min_active_width_gain_pct": min(widths),
        "max_active_width_gain_pct": max(widths), "max_active_peak_j_change_pct": max(jpks),
        ...
        "peak_reconnection_rate_change_pct": _pct(recon_rate([c for _, c in pairs]), recon_rate([b for b, _ in pairs])),
        "final_center_to_shoulder_change_pct": _pct(final_c["center_to_shoulder_ratio"], final_b["center_to_shoulder_ratio"]),
