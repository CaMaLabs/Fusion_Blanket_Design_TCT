...
python3 -u tools/tct_mechanism_explorer/native_h006_amplitude_ultrarefinement_audit.py
RC=$?

OUT="validation_runs/m3dc1_tct_native_h006_amplitude_ultrarefinement"
SUMMARY="$OUT/preemptive_timing_amplitude_refinement_summary.json"

echo
if [[ -f "$SUMMARY" ]]; then
  echo "===== NATIVE t=0.06 AMPLITUDE ULTRA-REFINEMENT ====="
  jq '{
    classification,
  ...
  echo
  echo "===== ULTRA-FINE AMPLITUDE RANKING ====="
  jq '[.case_summaries[] | {
