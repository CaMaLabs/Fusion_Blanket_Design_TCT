...
python3 -u tools/tct_mechanism_explorer/native_shoulder_geometry_refinement_audit.py
RC=$?

OUT="validation_runs/m3dc1_tct_native_shoulder_geometry_refinement"
SUMMARY="$OUT/native_shoulder_geometry_summary.json"

echo
if [[ -f "$SUMMARY" ]]; then
  echo "===== NATIVE SHOULDER-GEOMETRY RESULT ====="
  jq '{
    classification,
  ...
  echo
  echo "===== NEAR-FRONTIER CASES ====="
  jq '[.case_summaries[]
    | select(.worst_Jpk_change_pct <= 0.13)
    | {
        case,
        shoulder_width,
        shoulder_delta,
  ...
  jq '[.zero_equivalence_checks[] as $g
    | $g.checks[] as $t
    | $t.by_metric | to_entries[]
    | select(.value.pass == false)
    | {
        shoulder_width: $g.shoulder_width,
        shoulder_delta: $g.shoulder_delta,
        time: $t.time,
        metric: .key,
        delta: .value.delta
      }
  ...
  echo "Inspect:"
  echo "/tmp/m3dc1_tct_native_shoulder_geometry_refinement_runs"
fi
