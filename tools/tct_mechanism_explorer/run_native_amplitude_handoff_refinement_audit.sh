...
python3 tools/tct_mechanism_explorer/native_amplitude_handoff_refinement_audit.py
RC=$?

OUT="validation_runs/m3dc1_tct_native_amplitude_handoff_refinement"
SUMMARY="$OUT/native_handoff_refinement_summary.json"

echo
if [[ -f "$SUMMARY" ]]; then
  echo "===== NATIVE AMPLITUDE-HANDOFF RESULT ====="
  jq '{
    classification,
  ...
  echo "Inspect:"
  echo "/tmp/m3dc1_tct_native_amplitude_handoff_refinement_runs"
fi
