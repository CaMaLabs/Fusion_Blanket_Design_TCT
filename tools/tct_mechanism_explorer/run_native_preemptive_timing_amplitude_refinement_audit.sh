...
python3 -u tools/tct_mechanism_explorer/native_preemptive_timing_amplitude_refinement_audit.py
RC=$?

OUT="validation_runs/m3dc1_tct_native_preemptive_timing_amplitude_refinement"
SUMMARY="$OUT/preemptive_timing_amplitude_refinement_summary.json"

echo
if [[ -f "$SUMMARY" ]]; then
  echo "===== NATIVE PREEMPTIVE TIMING-AMPLITUDE REFINEMENT ====="
  jq '{
    classification,
