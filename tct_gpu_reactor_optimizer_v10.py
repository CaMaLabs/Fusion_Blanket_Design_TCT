...
def neutron_transport(design):
    ...
    for layer in design["blanket_layers"]:
        ...
        absorb = flux * capture * thickness
        scatter_flux = flux * scatter * 0.5

        flux = flux - absorb + scatter_flux

        breeding += mat["breed"] * absorb * 0.65
...
def wall_loading(power, design):
    ...
    surface = 4 * math.pi * R * a * 0.45

    return power / surface

# ------------------------------------------------
# NEUTRON DAMAGE LIFETIME
# ------------------------------------------------

def neutron_lifetime(damage):

    return 20 / (damage * 8e4 + 0.02)
