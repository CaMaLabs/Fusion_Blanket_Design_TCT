...
def neutron_transport(design):
    ...
    for layer in design["blanket_layers"]:
        ...
        breeding += mat["breeding"] * thickness * flux * 0.85
...
def tct_confinement(design):
    ...
    base = B**2 * a * (1 + 0.5*kappa + 0.4*delta) * 0.09
...
def neutron_lifetime(damage):
    ...
    return 25 / (damage*2e5 + 0.002)

# ------------------------------------------------
# BOOTSTRAP CURRENT BALANCE
# ------------------------------------------------

def bootstrap_penalty(design):

    return (1-design["bootstrap_fraction"]) * design["current_drive_power"]

# ------------------------------------------------
# HEAT REMOVAL LIMIT
# ------------------------------------------------

def blanket_heat_penalty(power):

    if power > MAX_BLANKET_HEAT:
        return (power - MAX_BLANKET_HEAT)*0.4
...
def simulate(design):
    ...
    drive_penalty = bootstrap_penalty(design)

    wall = wall_loading(fusion,design)

    life = neutron_lifetime(damage)

    pnet = fusion*0.35 - pump - drive_penalty

    pnet -= blanket_heat_penalty(fusion)
