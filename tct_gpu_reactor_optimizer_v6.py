...
MAX_WALL_LOAD = 10     # MW/m²
TARGET_BOOTSTRAP = 0.75
...
def neutron_lifetime(damage):
    ...
    # simplified lifetime estimate
    return 1 / (damage * 1e6 + 1e-12)
...
def simulate(design):
    ...
    wall_load = wall_loading(fusion, design)

    lifetime = neutron_lifetime(damage)
    ...
    if wall_load > MAX_WALL_LOAD:
        penalty += 200

    bootstrap_penalty = abs(design["bootstrap_fraction"] - TARGET_BOOTSTRAP) * 20
...
def score(result, design):
    ...
    power_reward = result["pnet"] * 1e-2
    lifetime_reward = math.log(result["lifetime"] + 1)
...
def mutate_numeric(v, pressure):

    return v * random.uniform(1-pressure,1+pressure)

def mutate_layer(layer, pressure):

    if random.random() < 0.4:

        layer["material"] = random.choice(material_keys)

    layer["thickness"] = mutate_numeric(layer["thickness"], pressure)

# ---------------------------------------------

def mutate(parent, generation):

    pressure = BASE_MUTATION_RATE * (1 - generation / GENERATIONS)
    ...
    for key in child:
        ...
        if random.random() < pressure:
            ...
            child[key] = mutate_numeric(child[key], pressure)

    for layer in child["blanket_layers"]:

        if random.random() < pressure:

            mutate_layer(layer, pressure)
    ...
    for generation in range(GENERATIONS):
        ...
        for design,result in zip(population,results):
            ...
            print(
                ...
                "wall",round(result["wall_load"],2),
                "life",round(result["lifetime"],2),
        ...
        while len(new_population) < POPULATION:
            ...
            new_population.append(mutate(parent,generation))
