...
BASE_DIR = Path.cwd()
RUNS_DIR = BASE_DIR / "runs_evo_blanket"
...
MATERIALS = {
    ...
    "Li2O": {
        "density": 2.01,
        "elements": [("Li", 2.0, "ao"), ("O", 1.0, "ao")],
        "li_score": 0.80,
    },
    "Li4SiO4": {
        "density": 2.39,
        "elements": [("Li", 4.0, "ao"), ("Si", 1.0, "ao"), ("O", 4.0, "ao")],
        "li_score": 0.90,
    },
    "Li2TiO3": {
        "density": 3.43,
        "elements": [("Li", 2.0, "ao"), ("Ti", 1.0, "ao"), ("O", 3.0, "ao")],
        "li_score": 0.85,
    },
    # PbLi proxy as a simple eutectic-like atom ratio approximation
    "PbLi": {
        "density": 9.8,
        "elements": [("Pb", 0.83, "ao"), ("Li", 0.17, "ao")],
        "li_score": 0.55,
    },
    # FLiBe proxy
}
...
TOTAL_THICKNESS_CHOICES = [5.0, 10.0]
SPLIT_CHOICES = [
    (0.20, 0.30, 0.50),
    (0.25, 0.25, 0.50),
    (0.33, 0.33, 0.34),
...
GENERATIONS = 8
ELITE_KEEP = 8
CHILDREN_PER_GEN = POP_SIZE - ELITE_KEEP
...
def postprocess_case(run_dir: Path, design: dict):
    ...
    # lithium breeding proxy from layer composition + position
    split = design["split"]
    layer_names = [design["front"], design["middle"], design["back"]]
    li_proxy = (
        MATERIALS[layer_names[0]]["li_score"] * split[0] * 0.7 +
        MATERIALS[layer_names[1]]["li_score"] * split[1] * 1.0 +
        MATERIALS[layer_names[2]]["li_score"] * split[2] * 0.4
    )
    ...
    # reward lithium-containing breeders, but not enough to let bad shields win
    score /= (1.0 + 0.75 * li_proxy)
    ...
    if ok:
        try:
            ...
            print(
                ...
                f"li_proxy={row['li_breeding_proxy']:.6g}"
            )
...
def main():
    ...
    for gen in range(GENERATIONS):
        ...
        for i, design in enumerate(population):
            ...
            gen_rows.append(row)
            all_rows.append(row)
            write_csv(BASE_DIR / "evo_blanket_results.csv", all_rows)
        ...
        for r in good[:8]:
            print(
                ...
                f"li_proxy={r['li_breeding_proxy']:.6g}"
            )
    ...
    print("\nFinished. Wrote evo_blanket_results.csv")
