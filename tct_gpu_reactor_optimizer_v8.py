...
def mhd_drag(design):
    ...
    for layer in design["blanket_layers"]:
        ...
        drag += sigma * B**2 * v * layer["thickness"]
...
def beta_limit(design):

    B = design["magnetic_field"]

    pressure = B**2/(2*MU0)

    plasma_pressure = pressure * BETA_LIMIT
...
def simulate(design):
    ...
    plasma_pressure = beta_limit(design)
